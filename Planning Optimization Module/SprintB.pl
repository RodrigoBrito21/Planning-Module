...
% NÃ£o conseguimos ligar mas funciona
...
% Prioritize surgeries by duration (longer surgeries first)
prioritize_surgeries(LOpCode, PrioritizedLOpCode) :-
    findall((OpCode, TResult), (member(OpCode, LOpCode), surgery_id(OpCode, OpType), sumTemp(OpType, TResult)), LDurations),
    sort(2, @>=, LDurations, SortedLDurations), % Sort by duration descending
...
% Find the optimal time slot that minimizes idle time
...
% Heurística 1: Priorizar cirurgias maiores
schedule_largest_first(Room, Day) :-
    prepare_data(Room, Day),
    findall(OpCode, surgery_id(OpCode, _), LOpCode),
    prioritize_by_duration(LOpCode, DescendingLOpCode), % Ordenar por maior duração
    schedule_surgeries(DescendingLOpCode, Room, Day). % Chamar o agendamento 
    
% Heurística 2: Priorizar cirurgias menores
    schedule_smallest_first(Room, Day) :-
        prepare_data(Room, Day),
        findall(OpCode, surgery_id(OpCode, _), LOpCode),
        prioritize_by_duration(LOpCode, AscendingLOpCode, ascending), % Ordenar por menor duração
        schedule_surgeries(AscendingLOpCode, Room, Day).
...
% Ordenar cirurgias por duração
prioritize_by_duration(LOpCode, OrderedLOpCode, Order) :-
    findall((OpCode, Duration), (member(OpCode, LOpCode), surgery_id(OpCode, OpType), sumTemp(OpType, Duration)), LDurations),
    sort(2, @>=, LDurations, SortedLDurations), % Ordenar pela duração
    findall(OpCode, member((OpCode, _), SortedLDurations), OrderedLOpCode).

% Agendar as cirurgias na ordem definida
schedule_surgeries([], _, _).
schedule_surgeries([OpCode | Rest], Room, Day) :-
    surgery_id(OpCode, OpType),
    sumTemp(OpType, TResult),
    availability_operation(OpCode, Room, Day, LPossibilities, LDoctors),
    schedule_first_interval(TResult, LPossibilities, (Start, End)),
    update_agendas(OpCode, Room, Day, Start, End, LDoctors),
    schedule_surgeries(Rest, Room, Day).

% Atualizar agendas de médicos e sala de operações
update_agendas(OpCode, Room, Day, Start, End, LDoctors) :-
    retract(agenda_operation_room1(Room, Day, Agenda)),
    insert_agenda((Start, End, OpCode), Agenda, UpdatedAgenda),
    assertz(agenda_operation_room1(Room, Day, UpdatedAgenda)),
    insert_agenda_doctors((Start, End, OpCode), Day, LDoctors).